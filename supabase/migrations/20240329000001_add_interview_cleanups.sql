...
CREATE OR REPLACE FUNCTION update_interview_cleanups_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = now();
  RETURN NEW;
...
-- Create trigger for interview_cleanups
CREATE TRIGGER update_interview_cleanups_updated_at
  BEFORE UPDATE ON interview_cleanups
  FOR EACH ROW
  EXECUTE FUNCTION update_interview_cleanups_updated_at(); 