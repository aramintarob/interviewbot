...
export function InterviewSession({ isTestMode = false }: InterviewSessionProps) {
  ...
  async function handleEndSession() {
    try {
      ...
      if (conversationRef.current) {
        const audioBlob = await conversationRef.current.endConversation();

        // Save interview assets if we have audio
        if (audioBlob && storageServiceRef.current) {
          ...
          const session: IInterviewSession = {
            id: uuidv4(),
            candidateName: 'Test Candidate', // Replace with actual candidate name
            startTime: sessionStartTime,
            endTime: Date.now(),
            currentQuestionIndex: pastQuestions.length,
            sequence,
            responses: [], // You'll need to track responses
            status: 'completed'
          };
          ...
          await storageServiceRef.current.saveInterviewAssets(
            session,
            audioBlob,
            '' // Empty string for transcript, it will be generated by ElevenLabs
          );
        }
